  ...
  return (
   <Link to={`/bookDetails/${bookId}`}>
      ...
      <div className="flex gap-3 mt-3">
        {
            tags.map(tag => <p className="bg-green-50 px-2 rounded-3xl text-sm text-[#23BE0A] font-semibold" >{tag} </p>)
        }
      </div>
      <div className="card-body px-1">
        ...
        <div className="card-actions justify-between mt-2">
          ...
          <div className="flex items-center gap-2">{rating} <CiStar className="text-lg" /> </div>
        </div>
      </div>
    </div>
   </Link>
